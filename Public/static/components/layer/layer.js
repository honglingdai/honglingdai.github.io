...
Class.pt.offset = function (times) {
    ...
    var area = [layero.outerWidth(true), layero.outerHeight(true)];
...
layer.close = function(index){
    var timeOut = 200;
    var layero = $('#'+ doms[0] + index), type = layero.attr('type');
        layero.addClass("layui-anim-close");
    if(!layero[0]) return;
    if(type === ready.type[1] && layero.attr('conType') === 'object'){
        var layeroChild = layero.children(':not(.'+ doms[5] +')');
        setTimeout(function(){layeroChild.remove()},timeOut);
        for(var i = 0; i < 2; i++){
            var layerWrap = layero.find('.layui-layer-wrap').unwrap();
        }
    } else {
        //低版本IE 回收 iframe
        if(type === ready.type[2]){
        ...
        setTimeout(function(){layero[0].innerHTML = '';layero.remove()},timeOut);
    }
    var layerShadeIndex = $('#layui-layer-moves, #layui-layer-shade' + index);
    layerShadeIndex.fadeOut(timeOut);
    setTimeout(function(){layerShadeIndex.remove();},timeOut);
